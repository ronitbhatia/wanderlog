...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a page",
        ["🏠 Home", "➕ Add Trip", "🗺️ Trip Locations", "📸 Gallery"]
    )
...
def show_locations():
    ...
    if trips.empty:
        ...
        st.write("Here are all your trip locations:")
        
        for _, trip in trips.iterrows():
            with st.expander(f"📍 {trip['title']} - {trip['location']}"):
                st.write(f"**Location:** {trip['location']}")
                st.write(f"**Coordinates:** {trip['latitude']:.4f}, {trip['longitude']:.4f}")
                st.write(f"**Dates:** {trip['start_date']} → {trip['end_date']}")
                if trip['description']:
                    st.write(f"**Description:** {trip['description']}")
                
                # Show Google Maps link
                maps_url = f"https://www.google.com/maps?q={trip['latitude']},{trip['longitude']}"
